...
def solve(values, weights, capacity):
    ...
    perturbation_prob = 0.1  # Probabilidad de aplicar ILS
    
    while T > min_T:
        for _ in range(max_iterations_per_T):

            if random.random() < perturbation_prob:
                # Se aplica la ILS (perturbación masiva del 25% y búsqueda local)
                perturbed_solution = get_perturbed_solution(current_solution, 0.25)
                new_solution = local_search(perturbed_solution, values, weights, capacity)
            else:
                # Se aplica solamente un pequeño cambio local
