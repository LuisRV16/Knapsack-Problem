...
def solve(values, weights, capacity):
    ...
    while T > min_T:
        for _ in range(max_iterations_per_T):
            ...
            # Decidir cuál solución actualizar en el SA
            if value_ils > best_value and weight_ils <= capacity:
                best_solution = new_solution_ils[:]
                best_value = value_ils
                current_solution = new_solution_ils[:]
            elif value_wo_ils > best_value and weight_wo_ils <= capacity:
                best_solution = new_solution_wo_ils[:]
                best_value = value_wo_ils
                current_solution = new_solution_wo_ils[:]
            else:
                delta = max(value_ils, value_wo_ils) - compute_value(values, weights, current_solution)[0]
                if delta > 0 or math.exp(delta / max(1e-10, T)) > random.random():
                    current_solution = new_solution_ils[:] if value_ils > value_wo_ils else new_solution_wo_ils[:]
    ...
    with open("ils_vs_wo_ils_results.json", "w") as json_file:
